...
- app chrome: the shared app bar is fixed to the top edge, uses separate browser and app chrome icons, keeps home/theme/dropdown controls aligned to the same compact height and radius as the Story button, keeps breadcrumb separators icon-only, and hides on downward scroll so long-form reading gets more vertical space
- story surfaces: dynamic story backdrops switch behind the content from intersecting background blocks without blur/backdrop-filter effects, while in-flow background cards keep uncropped optimized WebP previews, dialogue cards use borderless 2x top-cropped portraits with body-first reading layouts, desktop sibling navigation uses compact wrapping badges with matching accent phase tones, and bottom story navigation provides previous/next movement inside the current group
