...
- app chrome: the shared app bar is fixed to the top edge, uses separate browser and app chrome icons, keeps home/theme/dropdown/search/notes controls aligned to the same compact height and radius as the Story button, keeps breadcrumb separators icon-only, and hides on downward scroll so long-form reading gets more vertical space
- search surfaces: the search page uses the same archive card language as group/story lists, with a compact token-styled search input, highlighted matching lines, story classification badges, and scroll-driven incremental result rendering instead of a heavy virtualized dependency
