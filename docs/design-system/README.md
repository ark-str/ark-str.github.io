...
- app chrome: the shared app bar is fixed to the top edge, uses a rounded-square browser tab icon plus a separate transparent app chrome icon, keeps home/theme/dropdown/search/notes/settings controls aligned to the same compact height and radius as the Story button, renders group breadcrumbs as bordered compact controls, keeps breadcrumb separators icon-only, and hides on downward scroll so long-form reading gets more vertical space
...
- story surfaces: dynamic story backdrops switch behind the content from intersecting background blocks without blur/backdrop-filter effects, in-flow background cards keep uncropped optimized WebP previews, matching top and bottom control stacks expose read toggling, transient AI summary generation, generated summary disclosure, and previous/next movement inside the current group, the lower control stack automatically marks the story read when reached, AI summary requests use speaker-aware story transcripts, API response failures expose the HTTP status code, AI summary output renders Markdown in a consistent characters/key-events/final-summary format, dialogue cards keep the original text-first card layout while speaker portraits render as 2x top-cropped images inside the existing portrait slot, desktop sibling navigation uses compact wrapping badges with matching accent phase tones, a fixed lower-right note button opens a desktop sidebar or mobile bottom sheet without a drag handle, and browser back closes the open note editor before route navigation
