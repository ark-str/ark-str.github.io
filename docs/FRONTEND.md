...
- read progress is toggled from story action rows, stored by `storyId`, shared across locales, and shown on group story cards; AI summary widgets render Markdown, send a speaker-aware story transcript, show HTTP response codes for API response failures, use a stable character/key-events/final-summary response format, and share one transient story-level result between the top and bottom action rows
