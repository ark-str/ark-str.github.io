...
- read progress is toggled from story control stacks, automatically marked read when the lower story control stack is reached, stored by `storyId`, shared across locales, and shown on group story cards; AI summary widgets render Markdown, send a speaker-aware story transcript, show HTTP response codes for API response failures, use a stable character/key-events/final-summary response format, and share one transient story-level result between the top and bottom stacks
...
- story pages with matching top and bottom control stacks containing read/AI summary actions, generated summary disclosure, and previous/next story navigation, plus desktop-only left-side group navigation, main reading column, and a floating top button
