...
- the search overview loads the current locale's bundled search index, supports `/search?q=...` deep links, runs debounced immediate search while typing, highlights the matched line in story result cards, and incrementally renders broad result sets as the user scrolls
