...
- mixed `CharacterCutin`, `character` / `Character`, and `charslot` tags can coexist, and each dialogue line chooses exactly one winning eligible speaker frame by highest priority then most recent update; fresh cutins can survive intervening unresolved frames long enough to confirm their first speaker, but confirmed cutins only override a newer non-cutin frame after that frame has been confirmed to a different speaker
...
- `focus < 0`, neutral `charslot` frames, and focus-only or effect-only `charslot` updates stay visible but do not create new fallback aliases; focused unresolved slots must not borrow older active non-cutin slots, effect-only updates cannot act as newer speaker takeovers, and speaker-name fallback after active frames clear is limited to confirmed fresh-frame `char_` operator aliases
...
- story pages include matching top and bottom control stacks for read state, one-shot AI summary generation, generated summary disclosure, and previous/next story navigation; AI summaries use a speaker-aware transcript of rendered story text, show HTTP response codes for API response failures, render Markdown output in a consistent characters/key-events/final-summary format, share the same transient state between the top and bottom widgets, are not persisted, and show a settings action when no Google AI Studio API key is saved
- story pages automatically mark the current story read when the bottom control stack enters the viewport, using one-way read completion rather than toggle behavior
...
- a fixed top app bar shared by home, locale archive, group, and story pages; it uses a rounded-square browser tab icon, a separate transparent app chrome icon, matched compact home/theme/dropdown control heights and radii, icon breadcrumb separators, icon-only theme control, mobile wrapping, and scroll-down hide / scroll-up reveal behavior
...
- story pages place desktop-only group story navigation on the left with viewport-constrained internal scrolling, hide that navigation on mobile, provide previous/next story navigation in both top and bottom control stacks, and keep a floating scroll-to-top action at the lower right
