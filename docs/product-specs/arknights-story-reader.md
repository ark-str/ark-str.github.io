...
- the `/search` overview supports `/search?q=<query>` deep links, debounced URL replacement while typing, highlighted matching story lines, story cards that route back to the matched story, and scroll-driven incremental rendering for broad one-character queries
