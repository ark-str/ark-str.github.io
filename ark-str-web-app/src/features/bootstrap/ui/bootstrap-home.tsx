      ...
      <section className="grid gap-6 lg:grid-cols-[1.35fr_0.95fr]">
        <Card className="overflow-hidden bg-[var(--surface)]/94 backdrop-blur">
          <CardHeader className="gap-4 pb-0">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <button
                ...
                className={cn(buttonVariants({ variant: "subtle" }), "min-w-36 justify-between")}
            ...
            <CardTitle className="max-w-4xl text-4xl leading-tight md:text-5xl">
              URL만 있으면 바로 명일방주 스토리를 읽을 수 있는 첫 번째 reader shell을 여는 중입니다.
            </CardTitle>
            <CardDescription className="max-w-3xl text-base leading-8 md:text-lg">
              이 단계에서는 canonical locale archive, story deep link, generated body JSON,
              그리고 localStorage 기반 reader session을 하나의 읽기 흐름으로 연결합니다.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-6 pt-8">
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-muted)] p-4">
                ...
                <p className="mt-1 text-sm leading-6 text-[var(--text-muted)]">
                  reader session이 있다면 마지막으로 읽던 URL로, 없다면 현재 locale의 대표
                  스토리로 이동합니다.
                </p>
              </div>
              <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-muted)] p-4">
                ...
                <p className="mt-1 text-sm leading-6 text-[var(--text-muted)]">
                  summary는 아직 생성되지 않았으므로, 리더에서 explicit empty state로만 노출합니다.
                </p>
              </div>
              <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-muted)] p-4">
                <p className="text-xs uppercase tracking-[0.22em] text-[var(--text-muted)]">
            ...
            <div className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--panel)] p-5">
                  ...
                  <div className="grid gap-2">
                    <label className="grid gap-2">
                      <span className="text-sm font-semibold text-[var(--text)]">
                        Preferred locale archive
                      </span>
                      <Select
                        data-testid="locale-select"
                        disabled={!isSessionHydrated}
                        onChange={(event) => setPreferredLocale(event.target.value as (typeof homeModel.locales)[number]["locale"])}
                        value={state.preferredLocale}
                      >
                        {homeModel.locales.map((locale) => (
                          <option key={locale.locale} value={locale.locale}>
                            {locale.label} · {locale.storyCount} stories
                          </option>
                        ))}
                      </Select>
                    </label>
                ...
                >
                  ...
                  {state.lastVisitedStory ? (
                    ...
                    <p className="mt-3 text-sm leading-6 text-[var(--text-muted)]">
                      아직 읽은 스토리가 없습니다. 아래 locale archive나 대표 스토리 카드에서
                      바로 시작할 수 있습니다.
                    </p>
                  )}
                </div>
            ...
            <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--panel)] p-4">
              ...
              <p className="mt-2 text-sm leading-7 text-[var(--text-muted)]">
                {readiness.generatedAt
                  ? `Generated at ${readiness.generatedAt}. story detail JSON과 summary 상태를 같이 읽어 리더를 구성합니다.`
      ...
      <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <Card className="bg-[var(--surface)]/92">
          <CardHeader className="flex-row items-start justify-between gap-4">
            <div className="space-y-2">
              ...
              <CardDescription>
                locale archive는 group과 story deep link의 출발점입니다.
              </CardDescription>
            </div>
            <button
        ...
        <Card className="bg-[var(--surface)]/92">
          <CardHeader>
            ...
            <CardDescription>
              이 단계는 summary와 character unlock이 아니라, 본문을 읽을 수 있는 shell을 여는
              단계입니다.
            </CardDescription>
          </CardHeader>
          <CardContent>
